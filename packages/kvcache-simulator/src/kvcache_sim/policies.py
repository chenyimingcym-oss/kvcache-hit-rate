...
def simulate_fifo(plan: ExecutionPlan, capacity: int, *, require_full_before_measurement: bool = True) -> PolicyResult:
    ...
    for request_index in range(plan.request_count):
        if require_full_before_measurement and request_index >= plan.warmup_requests and not full_before_measurement:
    ...
    if (require_full_before_measurement and not full_before_measurement) or total_tokens <= 0:
        return _underfilled("fifo", capacity, plan)
...
def simulate_trie_policy(
    plan: ExecutionPlan,
    capacity: int,
    *,
    optimal: bool,
    require_full_before_measurement: bool = True,
) -> PolicyResult:
...
def simulate_policy(
    ...
    *,
    ...
    if policy == "fifo":
        return simulate_fifo(plan, capacity, require_full_before_measurement=require_full_before_measurement)
    if policy == "lru":
        return simulate_trie_policy(
            plan,
            capacity,
            optimal=False,
