...
def _worker_simulate(task: tuple[str, int, bool]) -> tuple[tuple[str, int, bool], PolicyResult]:
    if _WORKER_PLAN is None:
        raise RuntimeError("Worker plan is not initialized")
    policy, capacity, require_full_before_measurement = task
    return task, simulate_policy(
        _WORKER_PLAN,
        policy,
        capacity,
        require_full_before_measurement=require_full_before_measurement,
    )
...
def _simulate_tasks(
    ...
    jobs: int,
) -> dict[tuple[str, int, bool], PolicyResult]:
    if not tasks:
        return {}
    if jobs <= 1 or "fork" not in mp.get_all_start_methods():
        return {
            task: simulate_policy(
                ...
                task[0],
                task[1],
                require_full_before_measurement=task[2],
            )
            for task in tasks
        }
...
def run_sweep(
    ...
    *,
    ...
    if backend == "cpp" and include_underfilled:
        raise ValueError("--include-underfilled is currently supported only by the Python backend")
    ...
    if backend == "cpp":
        ...
        require_full_before_measurement = not include_underfilled
        tasks: list[tuple[str, int, bool]] = []
        seen_tasks: set[tuple[str, int, bool]] = set()
        for _gib, capacity in capacities:
            if capacity <= 0:
                continue
            if unique_blocks > 0 and capacity > unique_blocks:
                break
            for policy in selected_policies:
                task = (policy, capacity, require_full_before_measurement)
    ...
    for gib, capacity in capacities:
        ...
        for policy in selected_policies:
            if capacity <= 0:
                ...
                result = sim_results[(policy, capacity, not include_underfilled)]
            else:
        ...
        if underfilled and not include_underfilled:
            break
    ...
    return {
        "metadata": {
            ...
            "measurementWindow": "all_requests" if warmup_fraction == 0 else ("last_50_percent_requests" if warmup_fraction == 0.5 else f"last_{max(0.0, 1.0 - warmup_fraction):.6f}_fraction_requests"),
            "underfilledBudgetPolicy": "include_from_empty_cache" if include_underfilled else "omit_budget_and_larger_points",
